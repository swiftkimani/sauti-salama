...
- [ ] Anthropic API key -> `ANTHROPIC_API_KEY`; send the five sample transcripts through `/webhooks/sms` and read the briefs in the console (check `provider: anthropic+rules`).
...
- [ ] Optional: OpenAI key for Whisper and one real recording through the sandbox voice.
